        ...
        return runWithSalesBotRuntime(
          ...
          async () => {
            ...
            try {
              ...
              if (update.message) {
                ...
                if (text.startsWith("/start")) {
                  ...
                  await sendMessage(
                    chatId,
                    "Confirme sua maioridade primeiro com /start.",
                    ageGate,
                  );
              ...
              if (update.callback_query) {
                ...
                if (data === "confirm_age") {
                  ...
                  const item = buttons.find(
                    (button) => button.id === data.slice("menu_item:".length),
                  );
                  if (item)
            ...
            async function showOffers(target: Target) {
              ...
              if (!offers.length)
                return render(target, "Nenhuma oferta ativa no momento.", backMenu);
            ...
            async function showContents(target: Target, categoryHash?: string) {
              {
                ...
                const selectedCategory =
                  categoryHash && categoryHash !== "all"
                    ? categories.find((category) => categoryKey(category.name) === categoryHash)
                        ?.name
                    : null;
                ...
                const visibleContents = selectedCategory
                  ? allContents.filter(
                      (content) => categoryName(content.category) === selectedCategory,
                    )
                  : allContents;
                ...
                const title = selectedCategory
                  ? `Categoria: ${escapeHtml(selectedCategory)}`
                  : "Todos";
                return render(
                  target,
                  `<b>Conteudos avulsos</b>\n\n${title}\nEscolha um item:`,
